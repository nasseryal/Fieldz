// Import des terrains multisports (city-stades, plateaux, etc.)
// Beaucoup de terrains de foot/basket sont classés dans cette catégorie
...
async function fetchPage(category, depCode, offset) {
  ...
  const params = new URLSearchParams({ where: whereClause, limit: '100', offset: String(offset) });
...
// Catégories supplémentaires à importer
const EXTRA_CATEGORIES = [
  { id: 'football', category: 'Multisports' },     // City-stades, plateaux multisports
  { id: 'football', category: 'Terrain de grand jeu' }, // Grands terrains
  { id: 'basket', category: 'Multisports' },
  { id: 'fitness', category: 'Fitness' },
  { id: 'fitness', category: 'Remise en forme' },
  { id: 'running', category: 'Course' },
  { id: 'skate', category: 'Skate' },
];

async function main() {
  console.log('🚀 Import complémentaire — catégories manquantes');
  console.log('='.repeat(50));

  let grandTotal = 0;

  for (const sport of EXTRA_CATEGORIES) {
    console.log(`\n🏟️  ${sport.id.toUpperCase()} — catégorie "${sport.category}"`);
    let sportTotal = 0;

    for (const dep of DEPS) {
      try {
        const firstPage = await fetchPage(sport.category, dep, 0);
        const total = Math.min(firstPage.total, 5000);
        if (total === 0) continue;
        ...
        while (offset < total) {
          const { records } = offset === 0 ? firstPage : await fetchPage(sport.category, dep, offset);
          if (records.length === 0) break;
          const spots = records.filter(r => r.equip_coordonnees).map(r => toSpot(r, sport.id));
          allSpots.push(...spots);
          offset += 100;
          await sleep(150);
        }
    ...
    console.log(`   ✅ ${sport.id} "${sport.category}" — ${sportTotal} spots`);
  ...
  console.log('\n' + '='.repeat(50));
  console.log(`🎉 TERMINÉ — ${grandTotal} spots supplémentaires importés`);
  process.exit(0);
}
